...
The player runs as a standalone console application more or less in background and is mainly controlled by the primary midi input device/controller. In addition, the computer keyboard controls sequence restart (CTRL+PAUSE/BREAK) and sequence exit (CTRL+C) flow control jumps.
...
The player exits with different return codes based on the exit scenario. This can be useful in case the player gets executed within a batch script. For instance one can put the player into a restart-loop in case the primary midi connection gets temporarly lost. This might be handy especially for live playing. Valid return codes are:

* `0` - regular exit: the player normally reached the sequence end
* `2` - error: the player stopped due to timeout (most likely lost midi equipment connection)
