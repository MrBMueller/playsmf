...
for (midi_file_event = MidiFile_getFirstEvent(midi_file); midi_file_event; midi_file_event = MidiFileEvent_getNextEventInFile(midi_file_event)) {
 ...
 if (MidiFileEvent_getType(midi_file_event) == MIDI_FILE_EVENT_TYPE_META) {
  ...
  MidiEvents[i].EventData = (unsigned long)MidiFileVoiceEvent_getData(midi_file_event);
 ...
 MidiEvents[i].EventIdx = (struct PNoteO*)(MidiEvents[i].Track<<11 | (MidiEvents[i].EventData & 0x7f00)>>8 | (MidiEvents[i].EventData & 0xf)<<7);
 ...
 if ((MidiEvents[i].EventData & 0xf0) == 0xb0) { MidiEvents[i].EventIdx = (struct PNoteO*)((unsigned long)MidiEvents[i].EventIdx + 1*TrkNum*16*128);
 ...
 MidiEvents[i].EventIdx = &PendingEventsO[(unsigned long)MidiEvents[i].EventIdx];
...
ExitVal |= 1; Exit2: ExitVal |= 2; Exit0: printf(" done. (%x)\n", ExitVal); Active = 0; for (i=0; i<(sizeof(Port2In)/sizeof(struct MidiIn)); i++) { if (Port2In[i].h) { midiInStop(Port2In[i].h); }} SetConsoleCtrlHandler(HandlerRoutine, FALSE); if (args[2] >= 0 && (args[2]&0xff) < 0xff) { timeEndPeriod(args[2]&0xff); }
