...
for (i=0; i<(sizeof(Port2In)/sizeof(struct MidiIn)); i++) { Port2In[i].StartTime = start_time; if (Port2In[i].h) { for (;;) { Port2In[i].StartTime = timeGetTime(); midiInStart(Port2In[i].h); if (timeGetTime() == Port2In[i].StartTime) { break; }}}} SetConsoleCtrlHandler(HandlerRoutine, TRUE);
