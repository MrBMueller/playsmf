...
for (i=0; i<=15; i++) { signed long z = -1;
 for (k=12; k+1<_msize(args)/sizeof(signed long); k+=args[k]>>16?1:7) { if (!(args[k]>>16)) { signed long C = args[6], Ck = args[k], Mk = args[k+1]+1, K2 = Ck-(2+MutesNum), K1 = K2-1, K0 = K1;
